...
for i in range(istart, 750):
    ...
    if i > 0:
        ...
        cmd = ["modify_wind" + data["SIROCCO_VER"], "-model_file", sir_model_file, "input"]
        logfile.write("Command line: " + ' '.join(cmd) + "\n")
        print("Command line: " + ' '.join(cmd) + "\n")
        ...
        cmd = ["mpirun", "-np", str(data["nproc_sir"]), "sirocco" + data["SIROCCO_VER"], "-f", "-r", "input.pf"]
    else:
        # First run of Python; no restart
        cmd = ["mpirun", "-np", str(data["nproc_sir"]), "sirocco" + data["SIROCCO_VER"], "-f", "input.pf"]
    ...
    # Process Python outputs to generate files needed by PLUTO
    cmd = ["rad_hydro_files" + data["SIROCCO_VER"], "input"]
    logfile.write("Command line: " + ' '.join(cmd) + "\n")
    print("Command line: " + ' '.join(cmd) + "\n")
    try:

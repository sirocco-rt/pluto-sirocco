...
/*! 
  \file  
  \brief General-purpose reconstruction algorithms

  Common-usgae reconstruction functions (not necessarily optimized)
  shared by:

  - Finite difference schemes  (fd_flux.c);
  - General array reconstructions in the transverse directions
    (array_reconstruct.c);
  - Some recently added reconstruction schemes (MP5, WENOZ).

  These functions returns only the right-extrapolated edge value
  \f$ V^L(x_{i+\HALF}) \f$.
  In order to obtain also \f$ V^R(x_{i-\HALF}) \f$ call the same
  function with array sorted in the reversed direction.
  ...
  \b References
     - "High-order conservative finite difference GLM-MHDschemes for
        cell-centered MHD" \n
 ...
 * \param [in]  dx  grid spacing (not used)
 ...
 *********************************************************************** */
{
  ...
  fMP = F[j] + MINMOD_LIMITER(F[j+1]-F[j],alpha*(F[j]-F[j-1]));
 ...
 *************************************************************** */
{
  ...
  dvc = 0.5*(dvm2 + dvm1);
  SM  = 2.0*MINMOD_LIMITER(dvm2, dvm1);
  Sm1 = MINMOD_LIMITER(dvc, SM);

  dvc = 0.5*(dvm1 + dvp1);
  SM  = 2.0*MINMOD_LIMITER(dvm1, dvp1);
  Sp1 = MINMOD_LIMITER(dvc, SM);

  dvc = 0.5*(dvp2 + dvp1);
  SM  = 2.0*MINMOD_LIMITER(dvp2, dvp1);
  Sp2 = MINMOD_LIMITER(dvc, SM);
 ...
 *************************************************************** */
{
  return (a + MINMOD_LIMITER(b-a,c-a));
}

...
/*!
  \file
  \brief Integration methods for cooling netowrk ODEs.
...
/*!
 * Explicit 5th order adaptive step size Cash-Karp integrator.
 * Attempt to integrate the cooling network with a single step
 * if the error is within the given tolerance.
 * Otherwise, reduce the time step and repeat the integration
 * as many times as necessary.
 *
 * \param [in]    v_in  initial condition array
 * \param [in]    k_in  right hand side, R(v_in)
 * \param [out]   v5th  final solution array
 * \param [in]    dt    the time step we want to achieve
 * \param [in]    tol   the relative tolerance
 * \param [in]   *vars  the list of dependent variables
 *
 * \return The suggested time step for the next iteration.
 *********************************************************************** */
{
  int   nv, ksub, kfail;
  ...
  double v1[NVAR_COOLING], v4th[NVAR_COOLING];
  double k2[NVAR_COOLING], k3[NVAR_COOLING], k4[NVAR_COOLING];
  double k5[NVAR_COOLING], k6[NVAR_COOLING];
  double vscal[NVAR_COOLING];
  double tsub[4096];
  double v0_copy[NVAR_COOLING], k1_copy[NVAR_COOLING];  
...
/* -------------------------------------------
  ...
  /* -----------------------------------
    ...
    vscal[RHOE] = fabs(v0[RHOE]) + dt*fabs(k1[RHOE]);
   ...
   /* -- do not take error on Fe -- */
    ...
    /* -- initialize solution vector before continuing -- */
      ...
      if (ksub > 1000) {
        printLog ("! SolveODE_CK45: Number of substeps too large (%d)\n",ksub);
        QUIT_PLUTO(1);
      }
  ...
  if (ksub > 100) {
//    printLog ("! SolveODE_CK45: number of substeps is %d\n", ksub);
  }

/* --------------------------------------------------------
   Restore input arrays inside v0 and k1
   -------------------------------------------------------- */
...
/* -------------------------------------------
  ...
  for (nv = 0; nv < NVAR_COOLING; nv++) v1[nv] = v0[nv];
...
double SolveODE_RKF12 (double *v0, double *k1, double *v2nd, 
                       double dt, intList *vars)
/*!
 * Explicit 2nd order integrator with 1st-order embedded solution.
 * Attempt to integrate the cooling network with a single step
 * if the error is within the given tolerance.
 * The error is computed from the difference between 1st and 2nd order
 * solutions.
 *
 * \param [in]    v0    initial condition array
 * \param [in]    k1    right hand side, R(v0)
 * \param [out]   v2nd  final solution array
 * \param [in]    dt    the time step we want to achieve
 * \param [in]   *vars  the list of dependent variables
 *
 * \return The (scaled) error between 1st and 2nd-order solutions
 * 
 *********************************************************************** */
{
  int  nv;
  double err, scrh, dt_grow;
  double v1[NVAR_COOLING], v1st[NVAR_COOLING], vscal[NVAR_COOLING];
  double k2[NVAR_COOLING];
...
double SolveODE_RKF23 (double *v0, double *k1, double *v3rd, 
                       double dt, intList *vars)
/*!
 * Explicit 3rd order integrator with 2nd-order embedded solution.
 * Attempt to integrate the cooling network with a single step
 * if the error is within the given tolerance.
 * \f[
 ...
 * \param [in]   *vars  the list of dependent variables
 *
 * \return The (scaled) error between 2nd and 3rd-order solutions
 *
 *********************************************************************** */
{
  int  nv;
  double err, scrh, dt_4, dt_6;
  double v1[NVAR_COOLING], v2nd[NVAR_COOLING], vscal[NVAR_COOLING];
  double k2[NVAR_COOLING], k3[NVAR_COOLING];
...
/* -------------------------------------------
  ...
  for (nv = 0; nv < NVAR_COOLING; nv++) v1[nv] = vbeg[nv] = v0[nv];
...
*/
  ...
  /* -- solve for g2 -- */
    ...
    v1[RHOE] = v0[RHOE] + A31*g1[n - 1] + A32*g2[n - 1];
  ...
  /* -- set right hand side for g3 -- */
    ...
    g3[n - 1] = k3[RHOE] + (C31*g1[n - 1] + C32*g2[n - 1])/dt;
  ...
  /* -- set right hand side for g4 -- */
    ...
    g4[n - 1] = k3[RHOE] + (C41*g1[n - 1] + C42*g2[n - 1] + C43*g3[n - 1])/dt;
  ...
  /* --  4th order solution & error estimation -- */
    ...
    i = n - 1;
    v4th[RHOE] = v0[RHOE] + B1*g1[i] + B2*g2[i] + B3*g3[i] + B4*g4[i];
    err      = fabs(E1*g1[i] + E2*g2[i] + E3*g3[i] + E4*g4[i])/vscal[RHOE];
    ...
    /* -- initialize solution vector continuing -- */
      ...
      if (ksub > 1000){
        printLog ("! SolveODE_ROS34: Number of substeps too large (%d)\n",ksub);
        QUIT_PLUTO(1);
      }
  ...
  if (ksub > 2) {
    int i;
    printLog ("! SolveODE_ROS34: number of substeps is %d\n", ksub);
/*
    for (i = 1; i <= ksub; i++){

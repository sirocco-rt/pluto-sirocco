...
#define MAX_NLEV  8   /* Maximum dimension of dynamically allocated arrays */
/* ********************************************************************* */
 ...
 *********************************************************************** */
{
  ...
  nlev = X->nlev;
  if (nlev > MAX_NLEV){
    printLog("! Solve_System(): nlev ( = %d) exceed MAX_NLEV ( = %d)\n", 
              nlev, MAX_NLEV);
    QUIT_PLUTO(1);
  }

  if (M == NULL){
    M    = ARRAY_2D(MAX_NLEV, MAX_NLEV, double);
    rhs  = ARRAY_1D(MAX_NLEV, double);
    p    = ARRAY_1D(MAX_NLEV, int);
    q    = ARRAY_2D(MAX_NLEV, MAX_NLEV, double);
  }

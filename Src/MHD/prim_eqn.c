...
/*!
  \file
  \brief Compute the right hand side of the primitive HD/MHD equations.
...
#if PHYSICS == MHD
  scrh     = v[BXt]*dv[BXt] + v[BXb]*dv[BXb];
#endif

  #if (EOS == IDEAL) || (EOS == PVTE_LAW)
  Adv[VXn] = v[VXn]*dv[VXn] + tau*(dv[PRS] + scrh);
//  Adv[VXn] = v[VXn]*dv[VXn] + (dv[PRS] + scrh)/v[RHO];
  #elif EOS == ISOTHERMAL
  Adv[VXn] = v[VXn]*dv[VXn] + tau*(cs2*dv[RHO] + scrh);
  #else
  printLog ("! PrimRHS(): not defined for this EoS\n");
  QUIT_PLUTO(1);
  #endif

#if PHYSICS == HD
  Adv[VXt] = v[VXn]*dv[VXt];
  Adv[VXb] = v[VXn]*dv[VXb];
#elif PHYSICS == MHD
...
/* -------------------------------------------------------- */
/*! \note
    In the 7-wave and 8-wave formulations we use the same
    matrix being decomposed into right and left eigenvectors
    during the Characteristic Tracing step.
    Note, however, that it DOES NOT include two additional
    terms  (-vy*dV[BX] for By, -vz*dv[BX] for Bz) that are
    needed in the 7-wave form and are added using source terms.
   -------------------------------------------------------- */
...
/* --------------------------------------------------------
  ...
  printLog ("! PrimSource: not implemented in Spherical geometry\n");
  QUIT_PLUTO(1);
...
/* --------------------------------------------------------
  ...
  if (g_dir == IDIR){
    for (i = beg; i <= end; i++) {
      src[i][VX1] +=  2.0*SB_OMEGA*gas->v[i][VX2];

    /* -- Add global pressure gradient -- */

      #if PARTICLES == PARTICLES_DUST
      src[i][VX1] += 2.0*SB_OMEGA*PARTICLES_DUST_SB_ETA_VK;
      #endif
    }
  } else if (g_dir == JDIR){
    for (j = beg; j <= end; j++) {
      #ifdef FARGO
      src[j][VX2] += (SB_Q - 2.0)*SB_OMEGA*gas->v[j][VX1];
      #else
      src[j][VX2] -= 2.0*SB_OMEGA*gas->v[j][VX1];
      #endif
    }
  }
  #if !INCLUDE_JDIR
  if (g_dir == KDIR){
    for (k = beg; k <= end; k++) {
      #ifdef FARGO
      src[k][VX2] += (SB_Q - 2.0)*SB_OMEGA*gas->v[k][VX1];
      #else
      src[k][VX2] -= 2.0*SB_OMEGA*gas->v[k][VX1];
      #endif
    }
  }
  #endif
  ...
  /* --------------------------------------------
     Add tangential components in 1D. We follow
     the same conventions used in rhs_source.c
     -------------------------------------------- */
...
/* --------------------------------------------------------
   3. MHD, div.B related source terms
      (see Section 3.1.1 of Mignone & Tzeferacos JCP (2010)
   -------------------------------------------------------- */
...
#ifdef GLM_MHD
{
  ...
  if (g_dir == IDIR){
    ...
    for (j = beg-1; j <= end; j++){
      A[j]  = 1.0;
      dV[j] = fabs(grid->x[IDIR][i])*grid->dx[JDIR][j];
    }
  }else if (g_dir == KDIR){
    for (k = beg-1; k <= end; k++){
      A[k]  = 1.0; // grid->A[JDIR][g_k][j][g_i];
      dV[k] = grid->dx[KDIR][k]; // grid->dV[g_k][j][g_i];
    }
  }

  #elif GEOMETRY == SPHERICAL
  printLog ("! PrimSource(): Div. Cleaning does not work in this configuration.\n");
  printLog ("!               Try RK integrator instead\n");
  QUIT_PLUTO(1);
  #endif
  ...
  for (i = beg; i <= end; i++){
    ...
    #if EOS == IDEAL
    scrh = v[VXn]*v[BXn] + v[VXt]*v[BXt] + v[VXb]*v[BXb];
...
/* --------------------------------------------------------
  ...
  #if GEOMETRY == POLAR || GEOMETRY == SPHERICAL
  printLog ("! PrimSource(): works only in Cartesian or cylindrical coords\n");
  printLog ("! Use RK instead\n");
  QUIT_PLUTO(1);
  #endif

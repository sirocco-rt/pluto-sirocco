...
/* --------------------------------------------------------
  ...
  /* --------------------------------------------
     3c. Compute states in the ** region
     -------------------------------------------- */
    ...
    /* ---------------------------------------
      ...
      for (nv = NFLX; nv--; ){  
        sweep->flux[i][nv] = 0.5*(fL[i][nv] + fR[i][nv]);

        sweep->flux[i][nv] -=   0.5*fabs(SL[i])*(usL[nv]  - uL[nv])
                              + 0.5*fabs(S1L)  *(ussl[nv] - usL[nv])
                              + 0.5*fabs(SM)   *(ussr[nv] - ussl[nv])
                              + 0.5*fabs(S1R)  *(usR[nv] - ussr[nv])
                              + 0.5*fabs(SR[i])*(uR[nv] - usR[nv]);
      }
      sweep->press[i] = 0.5*(ptR[i] + ptL[i]);
    }  /* end if (SL < 0, SR > 0) */
  } /* end for (i = beg, end) */

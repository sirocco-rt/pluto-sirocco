...
/* --------------------------------------------------------
  ...
  for (i = beg; i <= end; i++) {
    ...
    #endif
    flag[i] = 0;
// if (g_dir == IDIR && abs(i+1-32) < 2 && abs(g_j-32) < 2){
//   printf ("GFORCE(%d, %d); g_intStage = %d\n",i+1,g_j, g_intStage);
//   ShowState(stateLW.u[i],2);
// }
  }
...
/* --------------------------------------------------------
  ...
  /* ------------------------------------------------------
    ...
    sweep->dL[i] = 0.5*(om*dtdx*vL[VXn]*vxn_LW + (1.0 - om)*dxdt);
    sweep->dR[i] = 0.5*(om*dtdx*vR[VXn]*vxn_LW + (1.0 - om)*dxdt);

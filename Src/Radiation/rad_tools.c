...
/*!
  \file
  \brief Routines used by the radiation module.
 ...
 *********************************************************************** */
{
	...
	return g_idealGasConst * exp(log(prs)-log(rho)) ;
}
 ...
 *********************************************************************** */
{
  ...
  dt = g_reducedC * rad_data->dt ;
	...
	u2 = 0. ;
  for (i=0; i<comps; i++){
		u[i] = primvar[VX1+i] / g_radC ;
		u2  += u[i]*u[i] ;
	}
	
	/*-- Compute products of beta and the Eddington tensor --*/
  uuD = 0.;
  for (i=0; i<comps; i++ ){
    uD[i] = 0.;
    for (j=0; j<comps; j++ ){
	...
	/*-- Compute some constants used to calculate C and b  --*/
  rho = primvar[RHO] ;
  mdt_rho = -dt*rho ;
  B = Blackbody( GetTemperature(rho, primvar[PRS]) ) ;
  coeff1 = dt * abs_op * rho * B ;
	
	/*-- Compute coefficients of the system C.(E,F^i)=b  --*/ 	
  b[0] = rad_data-> Rini[0] + coeff1 ;	
  C[0][0] = 1.0 + mdt_rho * ( scat_op*( u2 + uuD ) - abs_op );

	for (i=0; i<comps; i++){
    b[i+1] = rad_data-> Rini[i+1] + coeff1 * u[i] ;

    C[0][i+1] = mdt_rho * u[i] * ( abs_op - scat_op ) ;
    C[i+1][0] = mdt_rho * ( tot_op * uD[i] + scat_op * u[i] ) ;

    for (j=i; j<comps; j++)
      C[i+1][j+1] = C[j+1][i+1] = mdt_rho * abs_op * 2.0 * u[i] * u[j] ;

    C[i+1][i+1] += 1.0 - mdt_rho * tot_op ;
  }
  ...
  /*-- Compute products of u and the Eddington tensor --*/
  uuD = 0.;
  for (i=0; i<comps; i++ ){
    uD[i] = 0.;
    for (j=0; j<comps; j++ ){

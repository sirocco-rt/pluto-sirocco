...
/*!
  \file 
  \brief  Newton's method (relativistic & nonrelativistic)
 ...
 *********************************************************************** */
{
  ...
  dt = rad_data-> dt ;
    ...
    /*-- Compute beta or 4-velocity and derived quantities --*/
    #if RADIATION_NR
    u2 = rad_data-> u2;
    #else
    ...
    consv[ENR] = x[0];
    #if RADIATION_NR
    consv[ENG] = rad_data->Ttot[0] - (g_radC/g_reducedC)*x[0];
    #else
    ...
    for (i=0; i<comps; i++){
      consv[FR1+i] = x[i+1] ;
      #if RADIATION_NR
      consv[MX1+i] = rad_data->Ttot[i+1] - x[i+1]/g_reducedC ;
      #else
  ...
  #if RADIATION_NR
  mf[0] = q 
       + v[RHO] * ( abs_op - tot_op * (u2 + uuD) ) * v[ENR]
       - v[RHO] * ( abs_op  - scat_op ) * uF ;
  #else
  mf[0] = q * gamma 
    ...
    #if RADIATION_NR
    consv[ENR] = (g_reducedC/g_radC)*(rad_data->Ttot[0] - consv[ENG]) ;
    #else
    ...
    mf[i+1] = q * u[i] 
      ...
      #if RADIATION_NR
      consv[FR1+i] = g_reducedC*(rad_data->Ttot[i+1] - consv[MX1+i]);
      #else
    ...
    x[0] = v [PRS] ;
    #if RADIATION_NR
    for (i=0; i<comps; i++) x[i+1] = v[VX1+i] ;
    #else
    gamma = consv[RHO]/v[RHO] ;
 ...
 *********************************************************************** */
{
  ...
  double rhogamma, rhoh, rhoe, vel2, kinb2, vB, B2 ;
  double th, th2, wt ;
  #if EOS == IDEAL
    double gmmr = g_gamma/(g_gamma - 1.0) ;
    double gmm1 = g_gamma - 1.0;
  #endif
  ...
  if (v[PRS] < 0.0) {
    ...
    /*-- Set velocities, beta and beta^2 --*/
    u2 = 0. ;
    for (i=0; i<comps; i++){
      v[VX1+i] = x[i+1] ;
      u[i] = x[i+1]/g_radC ;
    ...
    rad_data-> u2 = u2 ;
    vel2 = u2 * g_radC * g_radC ;
    
    /*-- Set radiation fields --*/
    #if PHYSICS == HD
    kinb2 = 0.5*v[RHO]*vel2 ;
    #else
    kinb2  = v[RHO]*vel2 + v[BX1]*v[BX1] + v[BX2]*v[BX2] + v[BX3]*v[BX3];
    kinb2 *= 0.5;
    #endif
    
    #if EOS == IDEAL
    v[ENR] = rad_data-> Ttot[0] - kinb2 - v[PRS]/gmm1; 
    #elif EOS == PVTE_LAW  
    status = GetPV_Temperature(v, &th);
    if (status != 0){
      th      = T_CUT_RHOE;
      v[PRS] = Pressure(v, th);
      x[0] = v[PRS];
    }
    rhoe = InternalEnergy(v, th);
  
    v[ENR] = rad_data-> Ttot[0] - kinb2 - rhoe;
  
    if (rhoe != rhoe){
      printLog ("! RadIterToPrim (): rhoe = %8.3e, T = %8.3e\n",rhoe, th);
      QUIT_PLUTO(1);
    }
    #endif
    
    v[ENR] *= (g_reducedC/g_radC) ;
    
    for (i=0; i<comps; i++)
      v[FR1+i] = g_reducedC * (rad_data-> Ttot[i+1] - v[RHO]*v[VX1+i]) ;
  ...
  if (v[ENR] < 0.0) {
    WARNING(
      print("! RadIterToPrim: Erad < 0 (%8.2e) during implicit step, ", v[ENR]);
      Where (pos, NULL);
    )			

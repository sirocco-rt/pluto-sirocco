...
/*!
  \file 
  \brief   Compute wave speeds for radiation transport.

  Rad_Speed() computes an estimate to the leftmost and rightmost
  wave signal speeds of the radiation block.
...
/* ----------------------------------------------
  ...
  for (i = beg; i <= end; i++) {
    ...
    #if RADIATION_NR
      SrL[i] = g_reducedC * MAX ( ll, -vLim );
      SrR[i] = g_reducedC * MIN ( lr , vLim );
    #else
      SrL[i] = MAX ( ll, -vLim );
      SrR[i] = MIN ( lr , vLim );
    #endif    
	...
	#else
    #if RADIATION_NR
    SrL[i] = g_reducedC * MIN(sl_min[i], sr_min[i]);
    SrR[i] = g_reducedC * MAX(sl_max[i], sr_max[i]);
    #else

...
/* --------------------------------------------------------
  ...
  for (i = beg; i <= end; i++) {
    ...
    NVAR_LOOP(nv){
      ...
      dv  = v[i+1][nv] - v[i][nv];
      vp[i][nv] = v[i][nv] + MINMOD_LIMITER(dvp, dv);

      dv  = v[i][nv] - v[i-1][nv];
      vm[i][nv] = v[i][nv] + MINMOD_LIMITER(dvm, -dv);
    }
  }
  ...
  for (i = beg; i <= end; i++) NVAR_LOOP(nv) vm[i][nv] = vp[i-1][nv];
  #endif
...
/* --------------------------------------------------------------
  ...
  /* -- Project to characteristic spcae -- */
    ...
    for (k = 0; k < NVAR; k++){
      dwp[k] = MINMOD_LIMITER(dwp[k],  dwp1[k]);
      dwm[k] = MINMOD_LIMITER(dwm[k], -dwm1[k]);
    }
  ...
  /* ------------------------------------------------------------------
    ...
    if (vp[i][RHO] < 0.0 || vm[i][RHO] < 0.0) {
      dvp[RHO] = 0.5*(MINMOD_LIMITER(dvF[i][RHO], dvF[i-1][RHO]));
      dvm[RHO] = - dvp[RHO]; 
    ...
    if (vp[i][PRS] < 0.0 || vm[i][PRS] < 0.0) {
      dvp[PRS] = 0.5*(MINMOD_LIMITER(dvF[i][PRS], dvF[i-1][PRS]));
      dvm[PRS] = - dvp[PRS];       
    ...
    if (vp[i][ENR] < 0.0 || vm[i][ENR] < 0.0) {
      dvp[ENR] = 0.5*(MINMOD_LIMITER(dvF[i][ENR], dvF[i-1][ENR]));
      dvm[ENR] = - dvp[ENR];       
      vp[i][ENR] = v[i][ENR] + dvp[ENR];
      vm[i][ENR] = v[i][ENR] + dvm[ENR];
    }
    #endif

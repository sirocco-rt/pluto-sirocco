...
static double BetaTheta(double, void *);
...
/* --------------------------------------------------------
   0. Allocate static memory areas.
   -------------------------------------------------------- */
...
/* --------------------------------------------------------
   1. Automatically detect whether the grid is regularly
    spaced or not.
  ...
  #else
  for (d = 0; d < DIMENSIONS; d++) uniform_grid[d] = grid->uniform[d];
  #endif

/* --------------------------------------------------------
   2. Set the index iL and iR defining the reconstruction
...
/* --------------------------------------------------------
  ...
  /* ---------------------------------------
     3a. Compute Q6 coefficients
     --------------------------------------- */

    PPM_Q6_Coeffs(s_hp3D[d], s_hm3D[d], d, grid);
    
  /* ---------------------------------------
     3b. If the grid is non-uniform, compute
    ...
    beg = iL;
    end = grid->np_tot[d] - 1 - iR;

  /* ---------------------------------------
     3c. Initialize coefficients using
         reconstruction weights for uniform
         Cartesian grid.
     --------------------------------------- */
  ...
  /* ---------------------------------------
     3d. Weights for cylindrical/polar
         geometries
     --------------------------------------- */
    ...
    #endif /* (GEOMETRY == CYLINDRICAL) || (GEOMETRY == POLAR) */
...
for (d = 0; d < DIMENSIONS; d++){
  ...
  wm = s_Wm3D[d];
  for (i = beg; i <= end; i++){
    printf ("%d  %12.6e  %12.6e  %12.6e %12.6e  %12.6e\n",i,
             wp[i][-2], wp[i][-1], wp[i][ 0],  wp[i][ 1],  wp[i][ 2]);
}
exit(1);
  */
...
/* ----------------------------------------------------
  ...
  beg = iL;
  end = grid->np_tot[dir] - 1 - iR;

  if (beta == NULL) {
    beta = ARRAY_2D(8, 8, double);
    print ("> PPM_FindWeights: computing PPM coefficients\n");
  }
    ...
    for (j = jb; j <= je; j++){       /* -- stencil loop -- */
      ...
      switch (grid_type) {
        ...
        case -1:
          ...
          for (s_k = 0; s_k < n; s_k++){
            double scrh;
            scrh = GaussQuadrature(&BetaTheta, NULL, rm, rp, 1, 5);         
            beta[s_k][j-jb] = scrh;         
          }
          beta[0][j-jb] /= vol;
          beta[1][j-jb] /= vol;
...
double BetaTheta(double x, void *q)
{
...
{
  if (s_Wp3D == NULL) {
    print ("! PPM_Coefficients: coefficients not set.\n");
    QUIT_PLUTO(1);
  }

...
#ifndef FD_ROE_AVERAGE
 #define FD_ROE_AVERAGE NO
#endif
...
/* --------------------------------------------------------
...
#else 
  for (i = beg; i <= end; i++){
    double wL = sqrt(v[i][RHO]);
    double wR = sqrt(v[i+1][RHO]);
    for (nv = NFLX; nv--;    ){
      Uave[i][nv] = (wL*u[i][nv] + wR*u[i+1][nv])/(wL + wR);
    }
    #ifdef GLM_MHD
    Uave[i][BXn]     = Vave[i][BXn]     = Bm[i];
    Uave[i][PSI_GLM] = Vave[i][PSI_GLM] = psim[i];
  ...
  ConsToPrim (Uave, Vave, beg, end, sweep->flag);
#endif
...
/* --------------------------------------------------------
  ...
  #if DIMENSIONS == 2
  cond =     (g_intStage == 1) && (g_dir == JDIR)
          && (g_i == IEND) && (g_stepNumber%FUNCTION_CLOCK_NSTEP == 0);
  #elif DIMENSIONS == 3
  cond =     (g_intStage == 1) && (g_dir == KDIR)
          && (g_i == IEND) && (g_j == JEND) 
          && (g_stepNumber%FUNCTION_CLOCK_NSTEP == 0);
  #endif
